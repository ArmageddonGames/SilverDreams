...
global script active{
	void run(){
		...
		while(true){
			...
			if ( Link->PressEx3 ) Game->PauseMusic(); 
			if ( Kink->PressEx4 ) Game->ResumeMusic();
			if ( Link->PressL ) {
...
item script brang8way{
	void run(){
		lweapon brang[8];
		for ( int q = 0; q < 8; q++ ) {
		brang[q] = Screen->CreateLWeapon(LW_BRANG);
		brang[q]->X = Link->X;
		brang[q]->Y = Link->Y;
		brang[q]->Dir = q;
		brang->Range = 50;
...
item script rangearrow{
	void run(){
		lweapon brang = Screen->CreateLWeapon(LW_ARROW);
		brang->X = Link->X;
		brang->Y = Link->Y;
		brang->Dir = Link->Dir;
		brang->Range = 20;
