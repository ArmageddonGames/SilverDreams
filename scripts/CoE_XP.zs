...
global script Init {
    void run(){
        RandomStats(MINSTAT);
        Game->MCounter[CR_LV] = VitalStats[ST_LEVEL];
        Game->Counter[CR_LV] = VitalStats[ST_LEVEL];
...
void RandomStats(int minimumValue){
    ...
    do {
        VitalStats[ST_STAMINA] = ((Roll(3,6))* 20);
    }
    while (VitalStats[ST_STAMINA] < minimumValue );
    
    Game->MCounter[CR_SP] = VitalStats[ST_STAMINA];
    Game->Counter[CR_SP] = VitalStats[ST_STAMINA];
    Game->MCounter[CR_MAXSP] = VitalStats[ST_STAMINA];
    Game->Counter[CR_MAXSP] = VitalStats[ST_STAMINA];
...
//Returns from VitalStats[]
float Stat(int var){
    return VitalStats[var];
}
...
//Sets a value in VitalStats[]
void Stat(int var, float value){
    VitalStats[var] = value;
}

//Easily set, change, or modify a value like this:
// Stat(ST_LUCK,20); //Changes Luck state to 20.

//Bumps a value in VitalStats by +1; boolean can be set to either true, or false, and is used as a signature ID.
void Stat(int var, bool increase){
    VitalStats[var]++;
}

//Bumps a value in VitalStats by arg amount; boolean can be set to either true, or false, and is used as a signature ID.
void Stat(int var, bool increase, int amount){
    VitalStats[var] += amount;
}
